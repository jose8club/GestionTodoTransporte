...
CREATE PROCEDURE `MATRICULAR` (
                            ...
                            telefono numeric,
                            email varchar(255),
                            tipoCliente varchar(10), #Franquicia Tributaria
                            licencia varchar(40), #Producto
                            fecha date, #de la transacción
                            usuario varchar(45), #extraer funcionario
                            curso varchar(6),
                            hraTeoria time,
                            hraPractica time,
                            #firmaG bool,
...
BEGIN
	...
	start transaction;
        ...
        select if( datediff((select FechaTermino from curso where curso = curso),fecha) > 0, true, false ) into cursoEnMarcha;
        
        if cuposLlenos or !cursoEnMarcha then 
			rollback;
			set error_msg = 'Cupos llenos';
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_msg;
		end if;
        
        insert into cliente (`Nombre`,`RUT`,`Fecha_Nac`,`Telefono`,`e-Mail`,`TipoCliente`)
					values (nombre, rut, fecha_Nac, telefono, email, tipoCliente);
		insert into compra (`Cliente`,`Producto`,`Fecha`)
					values (last_insert_id(), licencia, fecha);
        ...
        open clases;
			clases_loop: loop
				fetch clases into id_clase, tipoClase;
                    ...
                    if STRCMP(tipoClase, 'T') then 
						insert into teoria_estudiante (`Teoria`,`Estudiante`) values (id_clase, cod_matricula);
					elseif STRCMP(tipoClase, 'P') then
						insert into practica_estudiante (`Practica`,`Estudiante`) values (id_clase, cod_matricula);
					else
						rollback;
